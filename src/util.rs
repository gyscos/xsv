...
impl FilenameTemplate {
    ...
    /// Re-opens an existing, writable file in directory `path` with a filename
        ...
        P: AsRef<Path>,
    {
        ...
        let file = fs::OpenOptions::new().append(true).open(full_path)?;

        Ok(Config::new(&spath).from_writer(Box::new(file)))
    }
